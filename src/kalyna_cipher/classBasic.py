...
class classBasic:
    ...
    def func_get_column(self, input_matrix, input_int):
        ...
        for i in range(self.const_size):
            result.append(input_matrix[i][input_int])
        return result
    ...
    def func_int_to_column(self, input_int):
        # int
        val = "".join(
            ["0" for i in range(16 - len(hex(input_int)[2:]))]) + hex(input_int)[2:]
    ...
    def func_s_row(self, input_matrix):
        ...
        for i in range(self.const_size):
            ...
            result[i] = self.func_byte_cycle_shift_right(
                input_matrix[i], shift)
        return result
    ...
    def func_m_col(self, input_matrix):
        ...
        for i in range(self.const_size):
            for t in range(self.var_c):
                ...
                result[i][t] = reduce(
                    (lambda x, y: x ^ y), self.func_mul_vector(row, column))
        return result
    ...
    def func_shift_const(self, input_matrix, input_int):
        # [[], ....], int
        for i in range(self.var_c):
            column = self.func_column_to_int(
                self.func_get_column(input_matrix, i))
            self.func_set_column(
                input_matrix, self.func_int_to_column(column << input_int), i)
        return input_matrix
    ...
    def func_matrix_to_mas(self, input_matrix, input_int):
        ...
        mas = []
        for i in range(input_int):
            mas = mas + self.func_get_column(input_matrix, i)
        return mas
    ...
    def func_hard_print(self, input_matrix, input_str, input_int):
        ...
        print(result_str + "".join([" " for i in range(30 - len(result_str))]),
              self.func_matrix_to_string(input_matrix))

    def func_bit_cycle_shift_left(self, input_mas, input_int):
        # [], int
        result = ["".join(["0" for t in range(8 - len(bin(i)[2:]))]
                          ) + bin(i)[2:] for i in input_mas]
        result = [i[::-1] for i in result]
        result = "".join(result)
        for i in range(input_int):
            result = result + result[0]
            result = result[1:]
        result = [result[8 * i:8 + 8 * i][::-1]
