            ...
            if t == 0:
                ...
                if self.flag:
                    round.func_hard_print(
                        round.state, "func_r_add_rkey_round", t)
            if t != 0 and t != key.var_t - 1:
                round.func_xor_rkey_round(keys[t])
                if self.flag:
                    round.func_hard_print(
                        round.state, "func_xor_rkey_round", t)
                round.func_r_m_col_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_r_m_col_round", t)
                round.func_r_s_row_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_r_s_row_round", t)
                round.func_r_s_block_round()
                if self.flag:
                    round.func_hard_print(
                        round.state, "func_r_s_block_round", t)
            if t == key.var_t - 1:
                round.func_r_add_rkey_round(keys[t])
                if self.flag:
                    round.func_hard_print(
