    ...
    def func_encrypt(self, plaint_text_input, key_input):
        ...
        for i in range(key.var_t):
            if i == 0:
                ...
                if self.flag:
                    round.func_hard_print(
                        round.state, "func_add_rkey_round", i)
            if i != 0 and i != key.var_t - 1:
                round.func_s_block_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_s_block_round", i)
                round.func_s_row_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_s_row_round", i)
                round.func_m_col_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_m_col_round", i)
                round.func_xor_rkey_round(keys[i])
                if self.flag:
                    round.func_hard_print(
                        round.state, "func_xor_rkey_round", i)
            if i == key.var_t - 1:
                round.func_s_block_round()
                if self.flag:
                    round.func_hard_print(round.state, "func_s_block_round", i)
